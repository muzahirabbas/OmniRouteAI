...
const LUA_CHECK_CIRCUIT_BREAKER = `
  ...
  local disabledKey = KEYS[3]
  local threshold = tonumber(ARGV[1])
  local minSamples = tonumber(ARGV[2])
  local ttl = tonumber(ARGV[3])

  local successCount = tonumber(redis.call('GET', successKey) or 0)
  local failCount = tonumber(redis.call('GET', failKey) or 0)
  local total = successCount + failCount

  if total < minSamples then
    return 0
  end

  local errorRate = failCount / total
  if errorRate >= threshold then
    redis.call('SETEX', disabledKey, ttl, '1')
...
export async function getActiveProviders() {
  ...
  // Step 1: filter to active + not circuit-broken (parallelized Redis checks)
...
function weightedShuffle(providers) {
  ...
  const result    = [];
  // Fisher-Yates shuffle before weighted selection — eliminates insertion-order bias.
  // Array.sort(() => Math.random() - 0.5) is NOT uniformly random (V8 TimSort artifact).
...
export async function recordProviderResult(name, success) {
  ...
  // For local tools, we use a much shorter TTL (30s) so they recover faster
  // if the user just restarted the daemon or fixed a local config.
  const config = await getProviderConfig(name);
  const ttl = (config?.type === 'local_http') ? 30 : CIRCUIT_BREAKER_TTL;
  await checkCircuitBreaker(name, ttl).catch(() => {});
}
 ...
 * Uses atomic Lua script to prevent race conditions between
 * checking health counters and disabling the provider.
 *
 * Rules:
 ...
 *
 * @param {string} name
 * @param {number} [ttl] - disable duration
 */
async function checkCircuitBreaker(name, ttl = CIRCUIT_BREAKER_TTL) {
...
const result = await evalLua(
    ...
    console.warn(`Circuit breaker check failed for ${name}: ${err.message}`);
  }
}
